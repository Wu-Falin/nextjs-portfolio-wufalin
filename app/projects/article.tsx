...
export const Article: React.FC<Props> = ({ project, views, index }) => {
	return (
		...
		>
			...
			>
				<span className="on-field text-[11px] tabular-nums text-faint transition-colors duration-300 group-hover:text-accent">
					{String(index + 1).padStart(2, "0")}
				</span>

				<div className="min-w-0 flex-1">
					<h2 className="on-field text-xl font-bold tracking-[-0.02em] text-fg transition-colors duration-300 group-hover:text-accent sm:text-2xl">
						{project.title}
					</h2>
				...
				{views > 0 ? (
					<span className="on-field flex shrink-0 items-center gap-1 text-[11px] text-faint">
